<?php
/**
 * Created by PhpStorm.
 * User: alexeyp
 * Date: 31.07.14
 * Time: 2:55
 */
//


include_once '../config.php';
include_once 'LivelibBP.class.php';
include_once '../library/db.class.php';

$db = new db();
$bp = new LivelibBP($db);
//echo extension_loaded('tidy') ? "LOADED" : "NOT LOADED";
$sql = 'SELECT max(id) FROM livelib_urls WHERE type="book"';
$max_id = $db->select($sql, 'value');

$cur_id = 0;
$step = 1000;
//$i=7;
do
{
//    $sql = "truncate books; truncate mapping_links; truncate pub_houses; truncate reviews";
//    $db->exec($sql);

    $sql = 'SELECT id FROM livelib_urls WHERE type="book"  AND status="1" AND id >' . $cur_id . ' LIMIT ' . $step;
    //$sql = 'SELECT id FROM livelib_urls WHERE type="book"   AND id =207';

    echo $sql . "</br>";
    $url_ids = $db->select($sql, 'array');
    if(!empty($url_ids))
    {
        foreach($url_ids AS $url_id)
        {
            $cur_id = $url_id;
            $book = $bp->GetBookInfo($url_id);

            if($bp->SaveBook())
                $bp->ChangeUrlStatus($url_id, '2');
            else
                $bp->ChangeUrlStatus($url_id, '3');
        }
    }
    else
        $cur_id += $step;



  }while($cur_id<$max_id);

