...
include_once 'thelib.class.php';
include_once '../library/db.class.php';

$db = new db();
$bp = new ThelibBp($db);
//echo extension_loaded('tidy') ? "LOADED" : "NOT LOADED";
$sql = 'SELECT max(id) FROM litmir_urls WHERE type="book" AND status=1';
...
//    $db->exec($sql);
    ...
    $sql = 'SELECT id FROM thelib_urls WHERE type="book" AND  status=1 AND id >' . $cur_id . ' LIMIT ' . $step;
	//$sql = 'SELECT id FROM thelib_urls WHERE type="book" AND  id=41808';
